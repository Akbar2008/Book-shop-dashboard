...
export const Header = () => {
  const { UserData, callback, setCallback, productSold, setinputType } = useContext(Context);
  const [userSettings, setUserSettings] = useState(false);

  const animation = useSpring({
    maxHeight: userSettings ? 250 : 0,
    opacity: userSettings ? 1 : 0,
    overflow: "hidden",
  ...
  return (
    <header className='w-full flex bg-white p-6 shadow items-center justify-between'>
      <div className='w-full flex max-w-3xl border-gray-500 border-[1px] rounded-xl p-[10px_18px]'>
        <label htmlFor="search" className='mr-2 cursor-pointer'>
          <Search />
        </label>
        <input
          type="text"
          // onInput={(e) => { e.target.value = e.target.value.replace(/[0-9]/g, '') }}
          onInput={(e) => { setinputType(e.target.value) }}
          id="search"
          className='w-full outline-none bg-none'
          placeholder='Search interesting information or books'
        />
      </div>
      ...
      <Link to='/order' onClick={() => { setCallback(false) }} className="relative cursor-pointer mr-6">
        ...
        <div className='relative'>
          <button
            ...
            className='flex items-center gap-3 p-2 border rounded-lg shadow-sm hover:bg-gray-100 transition'
          >
            <div className="group relative">
              <p className="absolute hidden group-hover:block -top-8 -left-3 font-bold text-black ">{UserData.given_name || 'username'}</p>
              <img src={UserData.picture || "/image/user.png"} alt="user logo" className='w-8 h-8 cursor-pointer border-[1px] border-blue-100 rounded-[50%]' />
            </div>

            <Down className={`${userSettings ? 'rotate-180' : ''} transition-transform`} />
          </button>

          {/* Animatsiyalangan menyu */}
          <animated.div style={animation} className='absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-lg z-[1000]'>
